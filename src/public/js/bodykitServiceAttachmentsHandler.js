...
const bodykitServiceId = document.getElementById('bodykit-service-id').value;
...
formElem.addEventListener('submit', async (e) => {
  ...
  await fetch('/upload', {
    ...
    .then(async (response) => {
      ...
      const { bodykitServiceId } = await response.json();
      Swal.fire('Archivo subido!', 'El archivo ha sido subido a Google Drive.', 'success', {
        confirmButtonText: 'Ok',
        confirmButtonColor: '#10B981',
        iconColor: '#10B981',
        onclick: () => {
          window.location.href = `/servicios-carrocerias/${bodykitServiceId}`;
        },
        onclose: () => {
          window.location.href = `/servicios-carrocerias/${bodykitServiceId}`;
        },
