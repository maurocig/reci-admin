...
const bodyKitId = document.getElementById('bodyKitId');
  ...
  updateService(service).then((response) => {
    if (response.status === 200) {
      Swal.fire({
        ...
        if (result.value) {
          window.location = `/servicios-carrocerias/${serviceId}`;
        }
      });
    } else {
      Swal.fire({
...
async function updateService(data) {
  try {
    const response = await fetch(`/servicios-carrocerias/${serviceId}?_method=PUT`, {
      method: 'POST',
      headers: {
...
// only numbers in order number input
